...
import DifficultyCalculator20150404 from "./DifficultyCalculator.js";

class PPCalculator20150404 {
    ...
    calculate(beatmap, accuracyPercent = 100, mods = MODS_ENUM.None, combo = -1, misses = 0) {
        ...
        const diff = new DifficultyCalculator20150404().calculate(beatmap);
    ...
    calculateWithCounts(diff, beatmap, mods = MODS_ENUM.None, combo = -1, misses = 0, c300 = -1, c100 = 0, c50 = 0) {
        ...
        // Calculate Aim Value
        // -------------------
        let aimValue = Math.pow(5.0 * Math.max(1.0, aim / 0.0675) - 4.0, 3.0) / 100000;

        // Longer maps are worth more
	    let lengthBonus = 0.95 + 0.4 * Math.min(1.0, totalHits / 2000.0) + (totalHits > 2000 ? Math.log10(totalHits / 2000) * 0.5 : 0.0);
        ...
        if(approachRate > 10.33) {
            approachRateFactor += 0.45 * (approachRate - 10.33);
        ...
        // Longer maps are worth more
        speedValue *=  0.95 + 0.4 * Math.min(1.0, totalHits / 2000.0) + (totalHits > 2000 ? Math.log10(totalHits / 2000) * 0.5 : 0.0);
        ...
        let accValue = Math.pow(1.52163, overallDifficulty) * Math.pow(betterAccuracyPercentage, 24) * 2.83;
	    ...
	    let finalMultiplier = 1.12; // This is being adjusted to keep the final pp value scaled around what it used to be when changing things
...
export default PPCalculator20150404